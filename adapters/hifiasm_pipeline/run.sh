#!/usr/bin/env bash
# hifiasm adapter — HiFi-only assembly: fastplong trim -> hifiasm (primary
# contigs) -> gfatools gfa2fa -> AAFTF vecscreen/sourpurge/sort (contig-level
# cleanup chain; these AAFTF subcommands just take a FASTA, so they work
# regardless of which assembler produced it -- same tools the short-read
# aaftf/nf_aaftf adapters use for contamination screening).
#
...
#   $BENCH_OUTDIR/hifiasm_pipeline.log    step log (kept for debugging)
#   exit codes: 0=success, 10=partial, nonzero=failure
set -euo pipefail

echo "==> hifiasm_pipeline adapter: dataset=$BENCH_DATASET_DIR out=$BENCH_OUTDIR smoke=${BENCH_SMOKE:-0}"
...
WORK_DIR="${BENCH_OUTDIR}/hifiasm_work"
  ...
  cat > "${BENCH_OUTDIR}/run_manifest.json" <<JSON
{
  ...
  "provenance": {
    ...
    "pipeline": {
      ...
      "version_pin": "${HIFIASM_VERSION_PIN:-PENDING}",
...
if [ "${BENCH_SMOKE:-0}" = "1" ]; then
  printf '>contig_smoke_1\nACGTACGTACGTACGTACGTACGTACGT\n' > "${ASSEMBLY_OUT}/${DATASET_ID}_hifiasm_placeholder.fasta"
  RUN_UUID="smoke-hifiasm-${DATASET_ID}-$(date +%s)"
  OUTPUTS_JSON='[{"path": "assembly/'${DATASET_ID}'_hifiasm_placeholder.fasta", "md5": "'$(md5sum "${ASSEMBLY_OUT}/${DATASET_ID}_hifiasm_placeholder.fasta" | cut -d' ' -f1)'"}]'
  TRUTH_ACCESSION="none"
  finalize "success"
  echo "==> hifiasm_pipeline adapter done (smoke placeholder)."
  exit 0
fi
...
if [ -z "${AAFTF_SIF}" ] || [ ! -f "${AAFTF_SIF}" ]; then
  ...
  RUN_UUID="fail-hifiasm-${DATASET_ID}-$(date +%s)"; TRUTH_ACCESSION="none"
...
# ---------------------------------------------------------------------------
# Resolve params (threads / memory / trim / phylum). Defaults match
# configs/params/hifiasm.default.yaml.
# ---------------------------------------------------------------------------
read -r CPUS MEM_GB MINLEN MINQUAL PURGE_LEVEL AAFTF_DB SORT_MINLEN < <(python3 - "${BENCH_PARAMS:-}" <<'PY'
import os, sys
d = {}
...
print(t, m, trim.get("min_length", 1000), trim.get("min_quality", 10),
      asm.get("purge_level", 0), d.get("aaftf_db", "/bigdata/stajichlab/shared/lib/AAFTF_DB"),
      sort.get("min_contig_len", 500))
PY
)
: "${CPUS:=8}"; : "${MEM_GB:=32}"; : "${MINLEN:=1000}"; : "${MINQUAL:=10}"
: "${PURGE_LEVEL:=0}"; : "${AAFTF_DB:=/bigdata/stajichlab/shared/lib/AAFTF_DB}"; : "${SORT_MINLEN:=500}"
...
BASE="${DATASET_ID}_hifiasm"
LOG="${BENCH_OUTDIR}/hifiasm_pipeline.log"
: > "${LOG}"
...
# ---------------------------------------------------------------------------
# Assemble: hifiasm primary contigs, then gfatools gfa2fa.
# Our sim genomes are haploid, so no trio/Hi-C partitioning is used -- take
# the default primary-contig graph (<prefix>.bp.p_ctg.gfa).
# ---------------------------------------------------------------------------
echo "==> hifiasm: cpus=${CPUS} purge_level=${PURGE_LEVEL}" | tee -a "${LOG}"
START=$(date +%s)
set +e
(
  cd "${WORK_DIR}"
  singularity exec "${AAFTF_SIF}" hifiasm -o "${BASE}" -t "${CPUS}" -l "${PURGE_LEVEL}" "${TRIMMED}" >> "${LOG}" 2>&1
)
HIFIASM_EXIT=$?
set -e
PRIMARY_GFA="${WORK_DIR}/${BASE}.bp.p_ctg.gfa"
if [ ${HIFIASM_EXIT} -ne 0 ] || [ ! -s "${PRIMARY_GFA}" ]; then
  echo "ERROR: hifiasm failed (exit ${HIFIASM_EXIT}) or ${PRIMARY_GFA} missing; log tail:" >&2
  tail -30 "${LOG}" >&2 || true
...
PRIMARY_FASTA="${WORK_DIR}/${BASE}.p_ctg.fasta"
singularity exec "${AAFTF_SIF}" gfatools gfa2fa "${PRIMARY_GFA}" > "${PRIMARY_FASTA}" 2>> "${LOG}"
if [ ! -s "${PRIMARY_FASTA}" ]; then
  echo "ERROR: gfatools gfa2fa produced no output" >&2
  RUN_UUID="fail-hifiasm-${DATASET_ID}-$(date +%s)"; TRUTH_ACCESSION="none"
  finalize "failed"; exit 1
fi
...
singularity exec "${AAFTF_SIF}" AAFTF sourpurge -i "${PRIMARY_FASTA}" -o "${SOURPURGE_OUT}" \
...
OUT_FASTA="${ASSEMBLY_OUT}/${DATASET_ID}_hifiasm.fasta.gz"
...
RUN_UUID="hifiasm-${DATASET_ID}-$(date +%s)"
OUTPUTS_JSON='[{"path": "assembly/'${DATASET_ID}'_hifiasm.fasta.gz", "md5": "'${MD5}'", "size": '${SIZE}'}]'
TRUTH_ACCESSION="none"
finalize "success"
echo "==> hifiasm_pipeline adapter done: ${OUT_FASTA} (${SIZE} bytes) ${METRICS_JSON}"
